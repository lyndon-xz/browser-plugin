...
export function renderCardList(root, { cards, onDelete }) {
  let filter = ALL;

  function draw() {
    root.replaceChildren();

    // 0 张卡片时筛选条与列表都是死控件，只留一句结论加一句怎么做（DD-28）
    if (!cards.length) {
    ...
    for (const [value, label] of [[ALL, "全部"], ...projects.map((project) => [project, project])]) {
      const count = value === ALL ? cards.length : cards.filter((card) => card.source.project === value).length;
