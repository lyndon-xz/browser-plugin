...
/*
 * 失败态文案（design.md §4.4）：机器原因 + 人话结论 + 出路。
 * 错误不道歉、不含糊，直接说清发生了什么与下一步做什么。
 */
const EXPLANATION = {
  ...
  [ERROR_KIND.notFound]: {
    reason: "内容不存在",
    title: "这个文件或提交找不到了",
    detail: "评论指向的提交可能已被 force push 覆盖，或文件已改名。可以直接去 GitLab 看这条评论的原始位置。",
    canUseToken: false,
  },
  [ERROR_KIND.server]: {
    reason: "服务端出错",
...
// 认不出的错误也要有话说：不给出口的失败态等于把问题藏起来
const UNKNOWN = {
  reason: "未知错误",
  title: "这一步没能完成",
  detail: "插件收到了一个预料之外的错误。重试一次；持续失败请看浏览器控制台里 [review-lens] 开头的日志。",
  canUseToken: false,
};

export function renderFailure({ error, onRetry, onConfigureToken }) {
  ...
  reason.textContent = error.status ? `${error.status} · ${explanation.reason}` : explanation.reason;
  ...
  actions.append(actionButton("重试", onRetry));
  if (explanation.canUseToken) actions.append(actionButton("配置访问令牌", onConfigureToken));
