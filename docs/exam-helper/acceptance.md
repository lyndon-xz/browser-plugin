...
| 编号 | 所属模块 | 状态                | 留证                                       | 备注                                                                                                                                                                                                                    |
| ---- | -------- | ------------------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| V-6  | M3       | ✅ 通过（代码路径） | `evidence/screenshots/M3-V6-ai-answer.png` | 选中题库外题目（Python 装饰器）→ 气泡先 loading → AI 答案态「A C / 多选 / 🤖 AI 推理 / 解析」。因线上 key 失效，用本地 mock 桩返回 200 验证整条 content→background→fetch→parseResponse→渲染路径；真实模型答案需有效 key |
| V-7  | M3       | ✅ 通过             | 见备注                                     | 用**真实无效 key**（DeepSeek 返回 HTTP 401）端到端验证：未命中→loading→错误态「⚠ AI 请求失败，请检查网络连接」，不白屏不崩溃                                                                                            |

> **重要（待用户处理）**：需求 §8 写死的 DeepSeek key `sk-…2a87` 实测已失效（curl 与浏览器均返回 401 authentication_error）。M3 代码、错误兜底、渲染路径均已验证无误；要得到真实 AI 答案，请在 `exam-helper/background.js` 的 `DEEPSEEK_API_KEY` 替换为有效 key。deepseek.test.js 8 例 + 全套 60 单测通过。
