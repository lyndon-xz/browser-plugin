/**
 * 预置题库 EXAM_QUESTIONS（全量）
 *
 * 由 CSDN 真题博文（requirement.md §8 主来源）经 agent-browser 抓取后结构化生成，
 * 已按归一化题干去重、答案按原文（含订正）取值。
 *
  ...
  const EXAM_QUESTIONS = [
    {
      ...
      options: [
        ...
        { key: "C", text: "Timer运行多个TimeTask时，只要其中之一没有捕获抛出的异常，其它任务便会自动终止运行。" },
        { key: "D", text: "ScheduledExecutorService并发运行多个定时任务时，其中某线程抛出异常，不会影响到其它线程的继续运行。" },
      ],
      answer: ["B","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "表名、字段名禁止出现数字开头，禁止两个下划线中间只出现数字。" },
    ...
    },
    {
      ...
      answer: ["A"],
      explain: "HashMap的key/value均可以为null，但是TreeMap的key不能为空，value可以为空",
    },
    {
      id: 4,
      type: "multi",
      ...
      options: [
        ...
        { key: "C", text: "mvn dependency:tree -Dverbose -Dincludes=commons-lang 打印出与commons-lang相关的详细依赖。" },
        { key: "D", text: "mvn clean install 打印工程整个的依赖树结构，并部署到本地仓库中。" },
      ],
      answer: ["A","B","C"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "D", text: "不要使用一个常量类维护所有常量，应该按常量功能进行归类，分开维护。" },
      ],
      answer: ["A","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "C", text: "一般不要定义SimpleDateFormat的static变量，如果定义为static，必须保证线程安全。" },
        { key: "D", text: "推荐使用Apache封装好的DateUtils和DateFormatUtils工具类，来处理时间日期转换问题。" },
      ],
      answer: ["A","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "代码块缩进4个空格，如果使用tab缩进，请设置成1个tab为4个空格；" },
        { key: "B", text: "代码块缩进5个空格，如果使用tab缩进，请设置成1个tab为5个空格。" },
        { key: "C", text: "为了保持代码美观，《手册》强烈推荐增加若干空格，使某一行的变量与相邻对应位置的变量对齐。" },
        { key: "D", text: "方法体内的执行语句组、变量的定义语句组、不同的业务逻辑之间或者不同的语义之间推荐插入一个空行；" },
      ],
      answer: ["A","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "C", text: "对于转换过来的集合，它的 add/remove/clear 方法会抛出: UnsupportedOperationException。" },
        { key: "D", text: "Arrays.asList 体现的是适配器模式，只是转换接口，后台的数据仍是数组。" },
      ],
      answer: ["A","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "为防止obj对象本身空指针异常，书写代码时应该注意加异常捕获处理，例如：try { obj.method() } catch(NullPointerException e){……} 。" },
        { key: "B", text: "方法签名中，抛给调用者的关键字为throws" },
        { key: "C", text: "方法内部，抛出异常实例对象为throws" },
        { key: "D", text: "自定义异常要做到“认知对等”，即：抛出者和接收者要保持对自定义异常的认知统一，接收方需要知道这种异常的含义和对应的处理方案。" },
      ],
      answer: ["B","D"],
      explain: "【强制】Java 类库中定义的可以通过预检查方式规避的 RuntimeException 异常不应该通过",
    },
    {
      id: 10,
      type: "multi",
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "路径覆盖是最强覆盖，符合路径覆盖且测试全部通过，程序绝对没有问题。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "D", text: "NullPointerException , IllegalArgumentException属于unchecked exception。" },
      ],
      answer: ["B","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "【强制】包名统一使用小写，点分隔符之间有且仅有一个自然语义的英语单词，并且使用复数形式，例如：应用工具类包名为com.alibaba.mpp.utils" },
        { key: "B", text: "【强制】类名使用UpperCamelCase，必须遵从驼峰形式，但以下情形例外：（领域模型的相关命名）DO / DTO / VO / DAO等。" },
        { key: "C", text: "【强制】抽象类命名使用Abstract或Base开头；异常类命名使用Exception结尾；测试类命名以它要测试的类的名称开始，以Test结尾。" },
        { key: "D", text: "【强制】枚举类名建议带上Enum后缀，枚举成员名称需要全大写，单词间用下划线隔开。" },
        { key: "E", text: "如果使用到了设计模式，建议在类名中体现出具体模式。例如代理模式的类命名：LoginProxy；观察者模式命名：ResourceObserver。" },
      ],
      answer: ["B","C","D","E"],
      explain: "",
    },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "异常信息应该包括两类信息：案发现场信息和异常堆栈信息。" },
        { key: "B", text: "日志文件推荐至少保存15天，因为有些异常具备以“周”为频次发生的特点。" },
        { key: "C", text: "避免重复打印日志，浪费磁盘空间，务必在log4j.xml中设置additivity=false。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "查询语句WHERE date_format(gmt_create, ‘%Y-%m-%d’) = '2016-11-11’无法利用gmt_create索引。" },
        { key: "C", text: "当 c 列类型为 char 时，查询语句 WHERE c = 5 无法利用c索引。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "对varchar类型的字段建立索引，不需要指定索引长度，这样索引区分度最好。" },
        { key: "C", text: "业务上具有唯一特性的字段（含组合字段），必须指定唯一索引。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "对于Service类，内部的实现类加Impl的后缀与接口区别。例如：ProductServiceImpl实现ProductService接口。" },
        { key: "C", text: "对于Service类，基于SOA的理念，是以接口方式暴露服务。" },
        { key: "D", text: "尽量不在接口里定义变量，如果一定要定义变量，肯定是与接口方法相关，而且是整个应用的基础常量。" },
      ],
      answer: ["B","C","D"],
      explain: "JDK8 中接口允许有默认实现，那么这个 default 方法，是对所有实现类都有价值的默",
    },
    {
      id: 19,
      type: "single",
      ...
      options: [
        ...
        { key: "D", text: "POJO类的serialVersionUID不一致会抛出序列化运行时异常。" },
      ],
      answer: ["D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "向公司外部提供的http/api接口，推荐使用“错误码”方式返回异常或者错误信息。" },
        { key: "B", text: "对于应用内部的方法调用，推荐使用“抛出异常”的方式处理异常或者错误信息。" },
        { key: "C", text: "跨应用的RPC调用，推荐使用将“错误码”和“错误简短信息”封装成Result的方式进行返回。" },
        { key: "D", text: "对外提供的接口，一定要保证逻辑健壮性：尽量避免空指针等技术类异常；对于业务类异常要做好错误码或者异常信息的封装。" },
      ],
      answer: ["A","B","C","D"],
      explain: "",
    },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "分页查询，当统计的count为0时，应该直接返回，不要再执行分页查询语句。" },
        { key: "B", text: "iBATIS自带的queryForList(String statementName,int start,int size)分页接口有性能隐患，不允许使用。" },
        { key: "C", text: "定义明确的sql查询语句，通过传入参数start和size来实现分页逻辑。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "setter方法中，参数名称与类成员变量名称一致，this.成员名=参数名。" },
        { key: "C", text: "在getter方法中，尽量不要增加逻辑判断，因为添加了逻辑判断后，会增加排查问题难度。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "C", text: "如果是基本类型，推荐使用java.util.concurrent.atomic包下面提供的线程安全的基本类型包装类，例如AtomicInteger。" },
        { key: "D", text: "如果是集合，推荐使用java.util.concurrent提供的并发集合类，例如：ConcurrentHashMap。" },
      ],
      answer: ["B","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "运算符与下文一起换行，方法调用的点符号与下文一起换行。   .append()" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "SQL性能优化的目标：至少要达到 range 级别，要求是ref级别，如果可以是consts最好。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "关于explain的结果，type=index的索引效率好于type=ref。" },
        { key: "C", text: "sql查询条件 where a like ‘%阿里%’ ，不会走索引。" },
        { key: "D", text: "sql查询条件 where a like ‘阿里%’ ,a列创建了索引，一般会使用索引进行检索。" },
      ],
      answer: ["C","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "能够减少在创建和销毁线程上所花的时间以及系统资源的开销。" },
        { key: "B", text: "使用线程池一定能避免OOM问题。" },
        { key: "C", text: "线程资源必须通过线程池提供，不允许在应用中自行显式创建线程。" },
        { key: "D", text: "线程池能够根据资源等待情况，自动调整线程优先级并解决死锁问题。" },
      ],
      answer: ["A","C"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "如果日志中输出POJO类，POJO类需要重写toString方法，避免只输出hashCode。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "数据库中，表达是与否概念的字段，必须使用is_xxx的方式命名。" },
        { key: "B", text: "推荐使用iBATIS自带的queryForList(String statementName,int start,int size)进行分页查询。(不推荐)" },
        { key: "C", text: "为避免写resultMap，可以直接拿HashMap与HashTable作为查询结果集的输出。" },
        { key: "D", text: "不要用resultClass当返回参数，即使所有类属性名与数据库字段一一对应，也需要定义。" },
      ],
      answer: ["A","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "方法头定义签名上的注释可以使用//（双划线）简单说明，不必要遵守javadoc规范。（/** */）" },
        { key: "B", text: "类定义开始部分，一般都用Javadoc注释 程序的总体描述以及作者信息。" },
        { key: "C", text: "方法头定义签名上的注释必须遵守javadoc规范，使用/**回车来生成，不得在方法定义上方使用//（双划线）简单说明。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "常量的定义要力求语义表达完整清楚，让别人能从常量名称上大致了解含义，例如：MAX_STOCK_COUNT。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "C", text: "二方库里接口的返回值不能使用枚举类型，但可以包含枚举类型。" },
        { key: "D", text: "二方库里接口的返回值是枚举类型或包含枚举类型时，当二方库的枚举值升级（增加枚举值）时，可能会导致接口调用时出现枚举对象序列化异常。" },
      ],
      answer: ["A","B","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "hashcode决定（如：HashMap）存储位置；equals决定是否需要覆盖（同一hash下）集合元素。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "跨应用共享常量：放置在二方库中，通常是client.jar中的const目录下。" },
        { key: "B", text: "应用内共享常量：通常放置在一方库的子模块中的const目录下。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "tcp链接主动关闭的一方，在完成四次挥手协议后，即会立即关闭并释放socket。" },
        { key: "B", text: "处于time_wait状态的socket，其实是已经关闭状态，当需要新建连接时，可以被马上复用。" },
        { key: "C", text: "当大量socket处于time_wait状态时，会导致可用socket资源稀缺，从而导致服务器并发能力下降。" },
        { key: "D", text: "通过修改/etc/sysctl.conf配置文件，减小time_wait的超时时间，可以降低time_wait状态的socket数量，从而提升服务器并发能力。" },
      ],
      answer: ["C","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "捕获异常与抛异常，必须是完全匹配，或者捕获异常是抛异常的父类。" },
        { key: "C", text: "对大段代码进行try-catch，利用Throwable来捕捉，万无一失。" },
        { key: "D", text: "对大段代码进行try-catch，这是不负责任的表现，分清稳定代码和非稳定代码，对非稳定的代码做对应的异常处理。" },
      ],
      answer: ["B","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "高并发的业务场景下，要考虑加锁及同步处理带来的性能损耗，能用无锁数据结构，就不要用锁。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "D", text: "用户输入的SQL参数严格使用参数绑定或者METADATA字段值限定，防止SQL注入，禁止字符串拼接SQL访问数据库。" },
      ],
      answer: ["B","C","D"],
      explain: "",
    },
    {
      id: 40,
      type: "multi",
      title: "关于应用与数据库之间的操作，下列哪些说法符合《阿里巴巴Java开发手册》",
      options: [
        { key: "A", text: "对外提供一个大而全的接口进行POJO的update更新，这样比较省事，省代码。" },
        { key: "B", text: "使用事务回滚的地方需要考虑各方面的回滚方案，包括缓存回滚、搜索引擎回滚、消息补偿、统计修正等。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "依赖于一个二方库群时，必须定义一个统一版本变量，避免各子二方库版本号不一致。" },
        { key: "B", text: "可以允许子项目的pom依赖中出现相同的GroupId，相同的ArtifactId，但是不同的Version。" },
        { key: "C", text: "所有pom文件中的依赖声明放在语句块中，所有版本仲裁放在语句块中。" },
    ...
    },
    {
      id: 43,
      type: "multi",
      title: "关于工具类二方库已经提供的，尽量不要在本应用中编程实现，下列哪些说法符合《阿里巴巴Java开发手册》",
      options: [
        { key: "A", text: "json操作使用fastjson。" },
        { key: "B", text: "md5操作使用commons-codec。" },
        { key: "C", text: "ArrayUtils、NumberUtils、DateFormatUtils、DateUtils等优先使用org.apache.commons.lang包。" },
        { key: "D", text: "CollectionUtils优先使用org.apache.commons.collections4包。" },
      ],
      answer: ["A","B","D"],
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "类名使用UpperCamelCase命名法，但是领域模型命名除外，如：ProductDO的命名是规范的。" },
        { key: "B", text: "方法名命名应该使用lowerCamelCase命名法，如方法名：getHttpMessage() 是符合命名规范的。" },
        { key: "C", text: "为了方便理解，方法名或参数名可以使用拼音与英文混合的方式。" },
        { key: "D", text: "所有编程相关的命名均不能以下划线或美元符号开始，也不能以下划线或美元符号结束。" },
      ],
      answer: ["A","B","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "查询语句 where a = 5 and b between 5 and 10 可以使用该索引。" },
        { key: "C", text: "查询语句 where a in (5, 6, 7, 8, 9) and b = 5 可以使用该索引。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "Border，边界值测试，包括循环边界、特殊取值、特殊时间点、数据顺序等。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "数据对象命名：xxxDO，xxx即为数据表名，例如：ResellerAccountDO。" },
        { key: "B", text: "数据传输对象：xxxDTO，xxx为业务领域相关的名称，例如ProductDTO。" },
        { key: "C", text: "展示层对象：xxxVO，xxx一般为网页名称，例如RecommendProductVO。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "C", text: "客户的证件号码只显示第一位和最后一位，在必要业务场景下，经多因子强验证后可完整展示。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "线程资源必须通过线程池提供，不允许在应用中自行显式创建线程。" },
        { key: "B", text: "同步处理时，能锁部分代码区块的情况下不要锁整个方法；高并发时，同步调用应该考虑到性能损耗。" },
        { key: "C", text: "创建线程或线程池时，推荐给线程指定一个有意义的名称，方便出错时回溯。" },
        { key: "D", text: "推荐使用Executors.newFixedThreadPool(int x)生成指定大小的线程池。" },
      ],
      answer: ["A","B","C"],
      explain: "(线程池不允许使用 Executors 去创建，而是通过 ThreadPoolExecutor 的方式)",
    },
    {
      id: 53,
      type: "multi",
      ...
      options: [
        { key: "A", text: "对于“明确停止使用的代码和配置”，如方法、变量、类、配置文件、动态配置属性等要坚决从程序中清理出去，避免造成过多垃圾。" },
        { key: "B", text: "永久弃用的代码段注释掉即可，即不用加任何注释。" },
        { key: "C", text: "对于暂时被注释掉，后续可能恢复使用的代码片断，在注释代码上方，统一规定使用三 个斜杠(///)来说明注释掉代码的理由。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "使用Iterator方式，如果有并发，需要对Iterator对象加锁。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "C", text: "因为JAVA的自动装箱与拆箱机制，不需要根据场景来区分数据类型。" },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "主版本号:产品方向改变，或者大规模API不兼容，或者架构不兼容升级。" },
        { key: "C", text: "次版本号:保持相对兼容性，增加主要功能特性，影响范围极小的API不兼容修改。" },
        { key: "D", text: "修订号:保持完全兼容性，修复BUG、新增次要功能特性等。" },
      ],
      answer: ["A","B","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "待办事宜（TODO）:（ [标记人，标记时间，[预计处理时间]）" },
        { key: "C", text: "在注释中用FIXME标记某代码虽然实现了功能，但是实现的方法有待商榷，希望将来能改进" },
        { key: "D", text: "在注释中用FIXME标记某代码是错误的，而且不能工作，需要及时纠正的情况" },
      ],
      answer: ["A","B","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "C", text: "对于order by A或group by A语句，在A上建立索引，可以避免排序。" },
        { key: "D", text: "对于多列排序，需要所有所有列排序方向一致，才能利用索引。" },
      ],
      answer: ["A","B","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        ...
        { key: "D", text: "如果使用到了设计模式，建议在类名中体现出具体模式。例如代理模式的类命名：LoginProxy；观察者模式命名：ResourceObserver。" },
      ],
      answer: ["A","B","C","D"],
      explain: "",
    },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "POJO类中的任何布尔类型的变量，都不要加is，因为部分框架解析时有可能会出现序列化错误。" },
        { key: "B", text: "包名统一使用单数形式，如：com.alibaba.mpp.util。" },
        { key: "C", text: "中括号是数组类型的一部分，数组定义如下：String[] args; 不要误写为String args[]；" },
        { key: "D", text: "Service/DAO层方法命名可以参考规约，例如：删除的方法推荐使用remove或delete做前缀。" },
      ],
      answer: ["A","B","C","D"],
      explain: "",
    },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "D", text: "只要此二方库负责人保证说不会有任何影响，即可大胆升级，直接发布上线。" },
      ],
      answer: ["B"],
      explain: "【强制】二方库的新增或升级，保持除功能点之外的其它 jar 包仲裁结果不变。如果有改变，",
    },
    {
      id: 66,
      type: "multi",
      ...
      options: [
        { key: "A", text: "表字段注释，如果修改字段含义或对字段表示的状态追加时，需要及时更新。" },
        { key: "B", text: "合适的字符存储长度，不但节约数据库表空间、节约索引存储，更重要的是提升检索速度。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "数据库库名和表名没有规定，可任意取名，只要方便记忆即可。" },
        { key: "B", text: "库名应该尽量与应用名称保持一致，表的命名最好是业务名称_表名的方式。" },
        { key: "C", text: "无论是库名还是表名都禁用保留字，如desc、match、range等。" },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "返回的集合B没有实现Serializable接口，不能被序列化，所以不能应用于RPC场景。" },
        { key: "B", text: "在B集合中添加某个元素，那么A集合也会添加进去此元素。" },
        { key: "C", text: "集合A中元素的修改不会影响到集合B的任何操作。" },
        { key: "D", text: "对A元素个数的修改，会导致集合B的遍历产生ConcurrentModificationException 异常。" },
      ],
      answer: ["A","B","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "如果需要捕获不同类型异常，为了方便处理，可以使用catch(Exception e){...}。" },
        { key: "B", text: "不要捕获异常后不处理，丢弃异常信息。" },
        { key: "C", text: "捕获异常与抛异常，必须是完全匹配，或者捕获异常是抛异常的父类。" },
        { key: "D", text: "异常定义时区分unchecked / checked 异常，避免直接使用RuntimeException抛出。" },
      ],
      answer: ["B","C","D"],
      explain: "",
    },
    ...
    },
    {
      ...
      options: [
        ...
        { key: "B", text: "尽量减少try-catch 块内的逻辑，定义对象、变量、获取数据库连接等操作可以移到try-catch块外处理" },
        { key: "C", text: "if ( condition) statements; 单行语句不需要使用大括号。" },
        { key: "D", text: "在一个switch块内，都必须包含一个default语句并且放在最后，即使它什么代码也没有。" },
      ],
      answer: ["A","B","D"],
      explain: "",
    },
    ...
    },
    {
      ...
      options: [
        { key: "A", text: "禁止将URL、文件名、系统参数、数据库连接地址、业务规则的可变参数，硬编码在工程中。" },
        { key: "B", text: "long或者Long初始赋值时，必须是大写的L，不能小写。" },
        { key: "C", text: "当一个类有多个构造方法，或是多个同名方法，这些方法应该按顺序放置在一起，便于阅读。" },
        { key: "D", text: "相同参数类型，同等业务含义，才可以使用Java的可变参数，参数的类型尽量避免使用Object。" },
      ],
      answer: ["A","B","C","D"],
      explain: "",
    },
    {
      ...
      options: [
        { key: "A", text: "主键索引(primary key)，字段类型为unsigned bigint、单表时推荐自增、步长为1。" },
        { key: "B", text: "主键索引(primary key)，字段类型为unsigned bigint、单表时推荐自增、步长为2。" },
        { key: "C", text: "唯一索引（unique key），命名规则为uk_字段名（如果多个字段继续下划线）。" },
        { key: "D", text: "普通索引(normal index)，标记成idx_字段名（如果多个继续下划线）。" },
      ],
      answer: ["A","C","D"],
      explain: "",
    },
